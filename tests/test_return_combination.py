...
class TestExtremeGroupSpread:
    def test_default_is_extreme_group_spread(self):
        rets = _rets_df()
        out = BacktestExecutor().combine_portfolio_returns(rets, config={"breakpoint_quantiles": 10, "long_leg": "low"})
        # long=1 (0.01), short=10 (0.10) -> -0.09
        assert out["ls_return"].iloc[0] == pytest.approx(-0.09)

    def test_high_long_leg(self):
        rets = _rets_df()
        out = BacktestExecutor().combine_portfolio_returns(rets, config={"breakpoint_quantiles": 10, "long_leg": "high"})
...
class TestAverageLegSpread:
    def test_without_explicit_legs_matches_extreme_group_spread(self):
        ...
        out = BacktestExecutor().combine_portfolio_returns(rets, config)
        assert out["ls_return"].iloc[0] == pytest.approx(-0.09)
...
class TestFullPortfolioReturn:
    def test_returns_full_grid_unchanged(self):
        rets = _rets_df()
        out = BacktestExecutor().combine_portfolio_returns(rets, config={"return_combination_type": "full_portfolio_return"})
