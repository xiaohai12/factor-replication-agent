...
steps.compute_metrics() gains an optional `by_sample_period` split
...
def test_no_sample_window_omits_segmented_block():
    ...
    metrics = steps.compute_metrics(ls, config={})
...
def test_top_level_metrics_unchanged_with_window():
    ...
    base = steps.compute_metrics(ls, config={})
    withwin = steps.compute_metrics(
