...
plan.md Phase 6: `steps.load_daily_msf` (compounds daily CRSP-shaped data
into the standard monthly-keyed panel) and `steps.apply_excess_returns`
(subtracts rf when factor data is supplied).
...
class TestLoadDailyMsf:
    ...
    def test_missing_file_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            steps.load_daily_msf(tmp_path / "nope.parquet")
...
class TestApplyExcessReturns:
    ...
    def test_subtracts_rf_when_excess_and_factors_available(self):
        out = steps.apply_excess_returns(self._msf(), self._factors(), config={"return_basis": "excess"})
        assert out["ret"].tolist() == pytest.approx([0.04, 0.02])

    def test_defaults_to_excess_when_return_basis_unset(self):
        out = steps.apply_excess_returns(self._msf(), self._factors(), config={})
        assert out["ret"].tolist() == pytest.approx([0.04, 0.02])

    def test_noop_when_return_basis_raw(self):
        out = steps.apply_excess_returns(self._msf(), self._factors(), config={"return_basis": "raw"})
        pd.testing.assert_series_equal(out["ret"], self._msf()["ret"])

    def test_noop_when_no_factors(self):
        out = steps.apply_excess_returns(self._msf(), None, config={"return_basis": "excess"})
        pd.testing.assert_series_equal(out["ret"], self._msf()["ret"])

    def test_noop_when_factors_missing_rf_column(self):
        factors = pd.DataFrame({"yyyymm": [200001], "mktrf": [0.02]})
        out = steps.apply_excess_returns(self._msf(), factors, config={"return_basis": "excess"})
        pd.testing.assert_series_equal(out["ret"], self._msf()["ret"])
