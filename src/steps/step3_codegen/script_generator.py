...
The generated script is a THIN WRAPPER: it imports `BacktestEngine` and calls
`BacktestEngine.run_with_config()` for the 9-step lifecycle (missing policy ->
...
def generate_backtest_script(
    ...
    Args:
        ...
        ff_factors_path: Optional path to a Fama-French factor + rf parquet
            ...
            `BacktestEngine.run_with_config(..., factors=...)` so
    ...
    # Build config from MethodSpec — the SAME resolved config BacktestEngine.run()
...
This script is a thin wrapper around `BacktestEngine.run_with_config()` — it
...
# same text can be exec'd here AND passed to BacktestEngine as a PluginRecord
# (which loads *_hook functions the same way for both the in-process engine
# and this script — see BacktestEngine._load_hooks()).
# ===========================================================================
