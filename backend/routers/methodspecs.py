...
async def extract(req: ExtractRequest) -> dict:
    def run(log):
        ...
        log(f"Extracting MethodSpec for '{req.factor_id}'...")
        result = extractor.extract(factor_id=req.factor_id, paper_text=req.paper_text)
        if result.spec is not None:
            out_path = UNREVIEWED_DIR / f"{req.factor_id}.methodspec.json"
