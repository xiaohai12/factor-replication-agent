#!/usr/bin/env python3
"""Backtest Script — AssetGrowth

Factor ID: fae312b1730eefb6
...
CONFIG = {
    ...
    "unapplied_universe_filters": [{'concept_id': 'compustat_listing_duration', 'op': 'gte', 'value': 2, 'reason': 'no need'}],
...
OUTPUT_PATH = "/Users/shengzhaolei/Desktop/shengzhao lei/projects/factor-replication-agent/runs/backtest_scripts/results/fae312b1730eefb6/factorial_universe.csv"
...
PLUGIN_CODE = 'import pandas as pd\n\ndef compute_signal(df: pd.DataFrame) -> pd.DataFrame:\n    df = df.copy()\n    df = df.sort_values(["permno", "time_avail_m"])\n    df["assets_prior"] = df.groupby("permno")["at"].shift(1)\n    df["signal"] = (df["at"] - df["assets_prior"]) / df["assets_prior"]\n    df = df[\n        df["at"].notna()\n        & df["assets_prior"].notna()\n        & (df["at"] != 0)\n        & (df["assets_prior"] != 0)\n        & df["signal"].notna()\n        & df["signal"].replace([float("inf"), float("-inf")], pd.NA).notna()\n    ]\n    return df[["permno", "time_avail_m", "signal"]].rename(\n        columns={"time_avail_m": "yyyymm"}\n    )'

exec(compile(PLUGIN_CODE, "<plugin:fae312b1730eefb6>", "exec"), globals())
