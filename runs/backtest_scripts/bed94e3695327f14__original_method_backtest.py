#!/usr/bin/env python3
"""Backtest Script — zscore

Factor ID: bed94e3695327f14
...
CONFIG = {
    ...
    "universe_filters": [{'field': 'sic', 'op': 'intervals', 'value': [[1, 3999], [5000, 5999]]}, {'field': 'exchcd', 'op': 'in', 'value': [1, 2, 3]}],
    "unapplied_universe_filters": [],
    "universe_filter_join_sources": {'compustat_fundamental_annual': ['sic']},
...
SIGNAL_INPUT_SOURCES = {'compustat_fundamental_annual': ['wcap', 'at', 're', 'ebit', 'lt', 'sale'], 'crsp_msf': ['prc', 'shrout']}  # multi_source only: {source: [columns]}
CRSP_FISCAL_YEAR_END_COLUMNS = ['prc', 'shrout']  # match fiscal month through CCM, then apply accounting lag
OUTPUT_PATH = "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/runs/backtest_scripts/results/bed94e3695327f14/original_method.csv"
...
PLUGIN_CODE = 'import numpy as np\nimport pandas as pd\n\ndef compute_signal(df: pd.DataFrame) -> pd.DataFrame:\n    df = df.copy()\n    market_equity = df["prc"].abs() * df["shrout"] / 1000\n    df["signal"] = (\n        1.2 * (df["wcap"] / df["at"])\n        + 1.4 * (df["re"] / df["at"])\n        + 3.3 * (df["ebit"] / df["at"])\n        + 0.6 * (market_equity / df["lt"])\n        + (df["sale"] / df["at"])\n    )\n    df = df.replace([np.inf, -np.inf], np.nan).dropna(subset=["signal"])\n    return df[["permno", "time_avail_m", "signal"]].rename(\n        columns={"time_avail_m": "yyyymm"}\n    )'

exec(compile(PLUGIN_CODE, "<plugin:bed94e3695327f14>", "exec"), globals())
