...
CONFIG = {
    ...
    "universe_filters": [{'field': 'exchcd', 'op': 'in', 'value': [1, 2, 3]}, {'field': 'sic', 'op': 'gte', 'value': 1}, {'field': 'sic', 'op': 'lt', 'value': 6000}, {'field': 'sic', 'op': 'not_between', 'value': [4000, 4999]}],
...
SIGNAL_INPUT_SOURCES = {'compustat_fundamental_annual': ['wcap', 'at', 're', 'ebit', 'mkvalt', 'lt', 'sale']}  # multi_source only: {source: [columns]}
OUTPUT_PATH = "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/runs/backtest_scripts/results/ce8a2fdbb9ec12f2/original_method.csv"
...
PLUGIN_CODE = 'import numpy as np\nimport pandas as pd\n\ndef compute_signal(df: pd.DataFrame) -> pd.DataFrame:\n    df = df.copy()\n    required = ["wcap", "at", "re", "ebit", "mkvalt", "lt", "sale"]\n    df = df.dropna(subset=required)\n    df = df[(df["at"] != 0) & (df["lt"] != 0)]\n\n    df["signal"] = (\n        1.2 * (df["wcap"] / df["at"])\n        + 1.4 * (df["re"] / df["at"])\n        + 3.3 * (df["ebit"] / df["at"])\n        + 0.6 * (df["mkvalt"] / df["lt"])\n        + (df["sale"] / df["at"])\n    )\n\n    df = df[df["signal"].notna() & np.isfinite(df["signal"])]\n    return df[["permno", "time_avail_m", "signal"]].rename(\n        columns={"time_avail_m": "yyyymm"}\n    )'

exec(compile(PLUGIN_CODE, "<plugin:ce8a2fdbb9ec12f2>", "exec"), globals())
