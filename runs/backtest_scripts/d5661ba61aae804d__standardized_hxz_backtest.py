#!/usr/bin/env python3
"""Backtest Script — z_score

Factor ID: d5661ba61aae804d
Paper: Dichev, Ilia D. (1998). Is the Risk of Bankruptcy a Systematic Risk? The Journal of Finance, 53(3), 1131-1147.
...
Requirements:
  - pandas, numpy
  - CRSP monthly data at: /Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data/crsp_msf.parquet
    Expected columns: permno, yyyymm (or date), ret, me, shrcd, exchcd, siccd
  - Raw WRDS-shaped source tables under: /Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data
    Sources read: compustat_fundamental_annual, crsp_msf
"""
...
CONFIG = {
    ...
    "sample_start_year": 1981,
    "sample_end_year": 1995,
    "publication_year": 1998,
    "universe_filters": [{'field': 'exchcd', 'op': 'in', 'value': [1, 2, 3]}, {'field': 'siccd', 'op': 'not_between', 'value': [6000, 6999]}, {'field': 'ceq', 'op': 'gt', 'value': 0}],
    "unapplied_universe_filters": [{'concept_id': 'sic_code', 'op': 'in', 'value': [[1, 3999], [5000, 5999]], 'reason': 'The schema lacks an in_ranges operator; this structural substitute preserves the paper-stated union in the value but cannot express range semantics under the current enum.'}],
    ...
    "defaults_applied": [{'config_key': 'missing_action', 'value': 'drop', 'reason': 'MethodSpec field unspecified or off-menu; engine default applied', 'paper_value': 'unspecified'}, {'config_key': 'formation_month', 'value': 6, 'reason': 'MethodSpec field unspecified; engine default applied', 'paper_value': None}],
}

FACTOR_ID = "d5661ba61aae804d"
...
DATA_PATH = os.environ.get("BACKTEST_DATA_PATH", "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data/crsp_msf.parquet")
...
SIGNAL_DATA_DIR = os.environ.get("BACKTEST_SIGNAL_DATA_DIR", "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data")  # multi_source only: dir of raw WRDS-shaped tables
SIGNAL_INPUT_SOURCES = {'compustat_fundamental_annual': ['wcap', 're', 'ebit', 'at', 'lt', 'sale'], 'crsp_msf': ['prc', 'shrout']}  # multi_source only: {source: [columns]}
CRSP_FISCAL_YEAR_END_COLUMNS = ['prc', 'shrout']  # match fiscal month through CCM, then apply accounting lag
OUTPUT_PATH = "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/runs/backtest_scripts/results/d5661ba61aae804d/standardized_hxz.csv"
FF_FACTORS_PATH = "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data/local/ff_factors.parquet"  # optional; empty string if not supplied
LIQUIDITY_FACTORS_DATA_DIR = "/Users/shengzhaolei/Desktop/Shengzhao_Lei/sideProject/factor-replication-agent/data"  # optional; dir containing local/liquidity_factors.csv
...
PLUGIN_CODE = 'import pandas as pd\n\ndef compute_signal(df: pd.DataFrame) -> pd.DataFrame:\n    df = df.copy()\n    df["market_equity"] = df["prc"].abs() * df["shrout"] / 1000\n    df["signal"] = (\n        1.2 * (df["wcap"] / df["at"])\n        + 1.4 * (df["re"] / df["at"])\n        + 3.3 * (df["ebit"] / df["at"])\n        + 0.6 * (df["market_equity"] / df["lt"])\n        + (df["sale"] / df["at"])\n    )\n    df = df[\n        df["at"].notna()\n        & df["lt"].notna()\n        & (df["at"] != 0)\n        & (df["lt"] != 0)\n        & df["signal"].notna()\n    ]\n    df = df[df["signal"].map(lambda value: float("inf") > value > float("-inf"))]\n    return df[["permno", "time_avail_m", "signal"]].rename(\n        columns={"time_avail_m": "yyyymm"}\n    )'

exec(compile(PLUGIN_CODE, "<plugin:d5661ba61aae804d>", "exec"), globals())
